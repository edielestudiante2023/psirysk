<!-- PÁGINA: MAPA DE CALOR ESTRÉS FORMA B -->
...
<h2 class="section-title" style="color: #006699; text-align: center; text-decoration: underline; margin-bottom: 15px;">
    Mapa de Calor Síntomas de Estrés - Forma B
</h2>
...
<?php
if (!function_exists('getEstresColorB')) {
    function getEstresColorB($nivel) {
        $colores = [
...
if (!function_exists('getEstresTextColorB')) {
    function getEstresTextColorB($nivel) {
        return in_array($nivel, ['medio', 'riesgo_medio']) ? '#333' : '#fff';
    }
}
if (!function_exists('getNivelEstresTextoB')) {
    function getNivelEstresTextoB($nivel) {
        $textos = [
...
<div style="border: 2px solid #333; background: <?= getEstresColorB($totalData['nivel']) ?>; color: <?= getEstresTextColorB($totalData['nivel']) ?>; text-align: center; padding: 20px;">
    ...
    <div style="font-size: 14pt; font-weight: 600;"><?= getNivelEstresTextoB($totalData['nivel']) ?></div>
</div>
...
<div style="margin-top: 15px; padding: 10px; background: <?= in_array($totalData['nivel'], ['alto', 'muy_alto', 'riesgo_alto', 'riesgo_muy_alto']) ? '#ffebee' : '#e8f5e9' ?>; border-left: 3px solid <?= in_array($totalData['nivel'], ['alto', 'muy_alto', 'riesgo_alto', 'riesgo_muy_alto']) ? '#F44336' : '#4CAF50' ?>; font-size: 8pt;">
    ...
    El nivel de síntomas de estrés detectado es <strong><?= getNivelEstresTextoB($totalData['nivel']) ?></strong>.
...
<!-- Nota Metodológica -->
<div style="margin-top: 10px; padding: 8px; background: #fff3e0; border-left: 3px solid #FF9800; font-size: 7pt;">
...
<p style="text-align: center; color: #999; font-style: italic; padding: 40px 0;">
    No hay datos de estrés para Forma B disponibles.
</p>
<?php endif; ?>
