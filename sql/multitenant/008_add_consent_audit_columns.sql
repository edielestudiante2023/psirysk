...
ALTER TABLE `worker_demographics`
    ADD COLUMN `consent_text_hash` VARCHAR(64) NULL AFTER `consent_given_at` COMMENT 'SHA256 del aviso visto por el trabajador',
    ...
    ADD COLUMN `consent_tenant_id` INT UNSIGNED NULL AFTER `consent_user_agent` COMMENT 'Tenant del psicólogo responsable';

-- Backfill: para los registros existentes que ya tienen consent_given_at,
...
WHERE wd.consent_given_at IS NOT NULL AND wd.consent_tenant_id IS NULL;
