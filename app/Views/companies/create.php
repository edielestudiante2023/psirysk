            ...
            <div class="col-md-8">
                <div class="card shadow-sm">
                    <div class="card-header bg-primary text-white">
                        <h5 class="mb-0"><i class="fas fa-building me-2"></i><?= $title ?></h5>
                    </div>
                    <div class="card-body">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select class="form-select" id="type" name="type" required>
                                        ...
                                        <option value="gestor_multicompania" <?= old('type') === 'gestor_multicompania' ? 'selected' : '' ?>>
                                            Gestor Multicompañía
                                        </option>
                                        <option value="empresa_individual" <?= old('type') === 'empresa_individual' ? 'selected' : '' ?>>
                                            Empresa Individual
                                        </option>
                                    </select>
                                    <small class="text-muted">
                                        Gestor: administra varias empresas. Individual: empresa final
                                    </small>
                                </div>

                                <div class="col-md-6 mb-3">
                                    <label for="parent_company_id" class="form-label">Gestionada por (opcional)</label>
                                    <select class="form-select" id="parent_company_id" name="parent_company_id">
                                        <option value="">Sin gestor (cliente directo)</option>
                                        <?php foreach ($gestores as $gestor): ?>
                                            <option value="<?= $gestor['id'] ?>" <?= old('parent_company_id') == $gestor['id'] ? 'selected' : '' ?>>
                                                <?= esc($gestor['name']) ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                                    <small class="text-muted">
                                        Solo si es empresa gestionada por una administradora
                                    </small>
                                </div>
                            </div>
