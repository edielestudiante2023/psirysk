                    ...
                    <tbody>
                        ...
                        <tr data-worker-id="<?= $result['worker_id'] ?>">
                            ...
                            <td>
                                ...
                                <?php else:
                                    // Cliente - mostrar estado de solicitud
                                    $requestKey = $result['worker_id'] . '_estres';
                                    ...
                                    <?php elseif ($request['status'] === 'approved' && strtotime($request['access_granted_until']) > time()): ?>
                                        <a href="<?= base_url("individual-results/view/{$request['access_token']}") ?>"
                                           class="btn btn-sm btn-success" title="Ver resultados (acceso hasta <?= date('d/m/Y H:i', strtotime($request['access_granted_until'])) ?>)"
                                           target="_blank">
                                    ...
                                    <?php else: ?>
                                        <a href="<?= base_url("individual-results/request/{$serviceId}/{$result['worker_id']}/estres") ?>"
                                           class="btn btn-sm btn-secondary" title="Acceso expirado - Solicitar nuevamente">
...
function getActionButton(workerId) {
    ...
    const requestKey = workerId + '_estres';
    const request = accessRequests[requestKey];

    if (!request) {
        // Sin solicitud
        return '<a href="' + baseUrl + 'individual-results/request/' + serviceId + '/' + workerId + '/estres" class="btn btn-sm btn-primary" title="Solicitar acceso a resultados individuales">' +
            '<i class="fas fa-lock"></i> Solicitar' +
            '</a>';
    }
    ...
    if (request.status === 'approved') {
        ...
        if (expiresAt > now) {
            ...
            // Expirado
            return '<a href="' + baseUrl + 'individual-results/request/' + serviceId + '/' + workerId + '/estres" class="btn btn-sm btn-secondary" title="Acceso expirado - Solicitar nuevamente">' +
                '<i class="fas fa-redo"></i> Renovar' +
                '</a>';
        }
    }
