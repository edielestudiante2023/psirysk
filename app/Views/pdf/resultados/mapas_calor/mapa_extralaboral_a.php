<!-- PÁGINA: MAPA DE CALOR EXTRALABORAL FORMA A -->
...
<h2 class="section-title" style="color: #006699; text-align: center; text-decoration: underline; margin-bottom: 15px;">
    Mapa de Calor Extralaboral - Forma A
</h2>
...
<?php
if (!function_exists('getExtraColorA')) {
    function getExtraColorA($nivel) {
        $colores = [
...
if (!function_exists('getExtraTextColorA')) {
    function getExtraTextColorA($nivel) {
        return $nivel === 'riesgo_medio' ? '#333' : '#fff';
    }
}
if (!function_exists('getNivelExtraTextoA')) {
    function getNivelExtraTextoA($nivel) {
        $textos = [
...
<div style="border: 2px solid #333; background: #fff;">
    <div style="display: table; width: 100%;">
        ...
        <div style="display: table-cell; width: 40%; vertical-align: middle; text-align: center; padding: 20px; background: <?= getExtraColorA($totalData['nivel']) ?>; color: <?= getExtraTextColorA($totalData['nivel']) ?>; border-right: 2px solid #333; font-weight: bold; font-size: 10pt;">
            TOTAL FACTORES<br>EXTRALABORALES<br>
            <span style="font-size: 24pt;"><?= number_format($totalData['promedio'], 1) ?></span><br>
            <span style="font-size: 9pt;"><?= getNivelExtraTextoA($totalData['nivel']) ?></span>
        </div>
        ...
        <div style="display: table-cell; width: 60%; vertical-align: top;">
            ...
            ?>
            <div style="padding: 8px 12px; background: <?= getExtraColorA($dimData['nivel']) ?>; color: <?= getExtraTextColorA($dimData['nivel']) ?>; border-bottom: <?= $dimIndex < count($dimensiones) - 1 ? '1px solid rgba(0,0,0,0.2)' : 'none' ?>; font-size: 9pt; overflow: hidden;">
...
<!-- Interpretación -->
<div style="margin-top: 20px; padding: 12px; background: #e8f5e9; border-left: 3px solid #4CAF50; font-size: 9pt;">
    ...
    El nivel de riesgo detectado es: <strong><?= getNivelExtraTextoA($totalData['nivel']) ?></strong>.
</div>
...
<p style="text-align: center; color: #999; font-style: italic; padding: 40px 0;">
    No hay datos extralaborales de Forma A disponibles.
</p>
<?php endif; ?>
